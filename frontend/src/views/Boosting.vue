<script setup>
import Logo from '@/components/main/appbar-components/Logo'
import ClientAreaButton from '@/components/main/appbar-components/ClientAreaButton'
import SettingsButton from '@/components/main/appbar-components/SettingsButton'
import PurchaseEloBoostingButton from '@/components/main/appbar-components/PurchaseEloBoostingButton'
import LearnMoreButton from '@/components/main/appbar-components/LearnMoreButton'

import CurrentRank from '@/components/boosting/CurrentRank'

import divisions from '@/constants/leagueOfLegendsDivisions'

import divisionMileStones from '@/constants/leagueOfLegendsDivisionMileStones'


import { ref } from 'vue'

import { useCurrentOrder } from '@/store/currentOrder'
import { useAccount } from '@/store/account'

const useAccountStore = useAccount()


const CurrentOrderStore = useCurrentOrder()

const imgUrls = import.meta.glob('../assets/games/leagueOfLegends/divisions/*.png', {
  import: 'default',
  eager: true
})

const desiredDivision = ref(divisions[4])
const desiredDivisionMileStone = ref('I')


function changeDesiredDivision(newDivision) {
  desiredDivision.value = newDivision
}

function changeDesiredMileStone(newMileStone) {
  desiredDivisionMileStone.value = newMileStone
}

// constanta alinabilir ?
const options = [
  {
    label:'Stream',
    price: '%20',
    isActive: false
  }
]

function changeState(option){
  option.isActive = !option.isActive
}

async function createDivisionOrder()  {
  console.log('order created function called in Boosting.vue')


  await CurrentOrderStore.createOrder({
    customer: useAccountStore.user._id || 'will handle no user',
    gameType:'League Of Legends',
    orderType: 'divisions',
    desiredRank: {
      division: desiredDivision.value.name,
      mileStone: desiredDivisionMileStone.value
    },
    options: options,
  }
  )
}
</script>

<template lang="pug">
.body
    v-img.appBar(
    aspect-ratio='16/9'
    cover=''
    src='../assets/games/leagueOfLegends/talonSplash.png')
      .firstRow
          Logo()
          .buttons
            ClientAreaButton()
            SettingsButton()
      .trust-pilot ** TRUST PILOT 5 STAR REVIEW **
      .appbar-title LEAGUE OF LEGENDS
      .appbar-subtitle No time? Don’t worry. Let our team climb for you
      v-select.selectBoost(
        label="BOOST TYPE"
        :items="['Division', 'Levelling', 'Clash']"
        variant="solo"
        )
    .make-order
        CurrentRank()
        .desired-rank
            .title DESIRED RANK
            .divisions
                v-btn.division(
                    v-for='division in divisions'
                    :key='division.name'
                    @click="changeDesiredDivision(division)"
                    )
                    v-img.division(
                        :src='imgUrls[`../assets/games/leagueOfLegends/divisions/${division.name}.png`]'
                        :alt='division.name'
                        )
            .mileStones
                v-btn.mileStone(
                    v-for='milestone in divisionMileStones'
                    :key='milestone'
                    variant='tonal'
                    @click="changeDesiredMileStone(milestone)"
                    ) {{ milestone }}
            .selections
                v-select.selection(
                    label="Queue"
                    :items="['Solo/Duo','Flex']"
                    variant="solo"
                    v-model="CurrentOrderStore.queueType"
                    )
        .checkout
            .title CHECKOUT
            .fromTo
                .checkout-division
                    v-img(
                        :src='imgUrls[`../assets/games/leagueOfLegends/divisions/${CurrentOrderStore.division}.png`]'
                    )
                    .league
                        .d {{ CurrentOrderStore.division }}
                        .checkout-milestone {{ CurrentOrderStore.mileStone }}

                .polygon
                    v-img(
                        src='../assets/general/polygon.png'
                        )
                .checkout-division
                    v-img(
                        :src='imgUrls[`../assets/games/leagueOfLegends/divisions/${desiredDivision.name}.png`]'
                        )
                    .league
                        .d {{ desiredDivision.name }}
                        .checkout-milestone {{ desiredDivisionMileStone }}
            .options
                .option(
                    v-for='option in options'
                    :key="option.label"
                    )
                    v-switch(
                        flat
                        inset
                        :@click='changeState(option)'
                        color="success"
                        )
                    .text {{ option.label}}
                    .questionMark
                        v-img(
                            cover
                            src='../assets/general/questionMark.png'
                        )
                    .percentage {{ option.price }}
            .total-price
                .total TOTAL PRICE:
                .price 48.34$
            v-btn(
                @click='createDivisionOrder()'
            ) CREATE ORDER
</template>

<style scoped>
.body {
    font-family: Inter;
}
.appBar {
    padding: 3.2rem;
    display: flex;
}
.firstRow {
    display: flex;
    justify-content: space-between;
}
.trust-pilot {
    text-shadow: 4px 4px 0 rgba(0, 0, 0, 0.5);
    font-size: 24px;
    font-weight: bold;
    color: #fff;
}
.appbar-title {
    text-shadow: 4px 4px 0 rgba(0, 0, 0, 0.5);
    font-size: 48px;
    font-weight: bold;
    color: #fff;
}
.appbar-subtitle {
    text-shadow: 4px 4px 0 rgba(0, 0, 0, 0.5);
    font-size: 32px;
    font-weight: 500;
    color: #fff;
}
.selectBoost {
    width: 13rem;
    color: #974444;
}
.make-order {
    padding: 4rem;
    display: grid;
    width: 100%;
    height: 800px;
    background-color: rgba(239, 235, 234, 0.69);
    grid-template-columns: repeat(13,1fr);
    grid-gap: 3rem;
    grid-template-rows: repeat(2,1fr);
    grid-template-areas:
    "currentRank currentRank currentRank currentRank currentRank currentRank currentRank currentRank checkout checkout checkout checkout checkout"
    "desiredRank desiredRank desiredRank desiredRank desiredRank desiredRank desiredRank desiredRank checkout checkout checkout checkout checkout";
}
.desired-rank{
    display: flex;
    flex-direction: column;
    gap: 1rem;
    border-radius: 15px;
    box-shadow: 4px 4px 4px 0 rgba(0, 0, 0, 0.25);
    background-color: #fff;
    grid-area: desiredRank;
    padding: 2.5rem;
}
.title {
  font-size: 36px;
  font-weight: 900;
  line-height: normal;
  color: #000;
}
.divisions {
    display: flex;
    gap: 1rem;
}
.division {
    width: 4rem;
    height: 4rem;
    border-radius: 15px;
    background-color: #d9d9d9;
}

.mileStones {
    display: flex;
    gap: 0.75rem;
}
.mileStone {
    width: 45px;
    height: 45px;
    border-radius: 6px;
    background-color: #d9d9d9;
    font-weight: 900;
}

.selections {
    display: flex;
    gap: 1rem;
}
.selection {
    width: 13rem;
}
.checkout {
    border-radius: 15px;
    box-shadow: 4px 4px 4px 0 rgba(0, 0, 0, 0.25);
    background-color: #fff;
    grid-area: checkout;
    display:flex;
    flex-direction: column;
}
.polygon {
    width: 1rem;
}
.fromTo {
    height: 136px;
    background-color: #e4e4e4;
    display: flex;
    align-items: center;
    justify-content: space-around;
}
.checkout-division {
    width: 18%;
}
.options {
    width: 100%;
}
.option {
    display: flex;
    align-items: center;
    justify-content: space-between;
}
.text {
  font-size: 20px;
  font-weight: bold;
  color: #000;
}

.questionMark {
    width: 1rem;
}
.total-price {
    display: flex;
    align-items: center;
    justify-content: space-around;
}

.total {
    font-size: 24px;
    font-weight: 900;
    color: #000;
}
.price {
  font-size: 40px;
  font-weight: bold;
  color: #000;
}
</style>
