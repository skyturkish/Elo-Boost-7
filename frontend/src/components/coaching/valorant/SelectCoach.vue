...
const filteredBoosters = computed(() => {
  const updatedList = useBoosters().boosters.filter((booster) => {
    return Object.keys(booster.permissions.coach.games).includes('valorant')
  }).filter((booster) => {
...
function setBoosterAndCloseDialog(booster) {
    currentValorantOrder.coach = booster
    dialog.value = false
}

function setBoosterNullAndCloseDialog() {
    currentValorantOrder.coach = null
    dialog.value = false
}
...
CheckoutSelectionColumn(toolTipText="You can choose your favorite coach" title="COACH")
  img.logo(v-if="currentValorantOrder.coach == null" src='https://storage.googleapis.com/divine-boost-bucket/assets/assets/icons/plus.webp' alt="plus")
  img.logo.booster-photo(v-else :src='currentValorantOrder.coach.photo' :alt="currentValorantOrder.coach.name")
  v-dialog(v-model='dialog' activator='parent' width='auto')
    v-card
      .row
        ...
        v-btn.remove-booster(v-if="currentValorantOrder.coach != null" @click="setBoosterNullAndCloseDialog") REMOVE COACH
      ...
      .boosters(v-if="filteredBoosters.length > 0")
        BoosterCard(v-for="booster in filteredBoosters" :key="booster")
          .content
            ...
            .booster-lanes
              img.booster-lane(v-for="lane in booster.notifications.booster.games['valorant'].roles" :key="lane" :src='`https://storage.googleapis.com/divine-boost-bucket/assets/assets/lanes/${lane}.webp`' :alt="lane")
